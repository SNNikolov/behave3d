...
Behave3d.controllerMenu.prototype.messages       = [
	"show",             // Show the menu
...
Behave3d.controllerMenu.prototype.events         = [
	...
	"inner_item_clicked", // Triggered on clicking a menu item of this menu or any of its submenus; event params = {menu: reference_to_menu, pos: index_of_selected_item, item: reference_to_DOM_element}
	
	// "show" and "hide" events are fired on the actions() controller
];
Behave3d.controllerMenu.prototype.default_params = {
	...
	for_each_menu      : "",     // Name of or reference to a callback function to be called for each submenu of this menu; callback(), where this = submenu's menu controller
};
...
Behave3d.controllerMenu.prototype.destruct = function() {
	for (var i = 0; i < this.active_event_listeners.length; i++) {
		...
		if (listener.condition)
			listener.element.removeEventListener(listener.type, listener.handle, listener.useCapture);
	}
};
...
Behave3d.controllerMenu.prototype.message = function(message, message_params)
{
	...
	if (message == "update") {
	...
	else if ((message == "show" || message == "show_immediately") && !this.menu_shown) {
		if (message_params.show_op_id !== undefined)
			if (this.show_operation_id != message_params.show_op_id)
				return; // Skip if there have been other show/hide operations meanwhile
		
		this.menu_shown = true;
		
		// Start transition or change display style if no transition
		if (this.owner.actions_controller.fireEvent(message) == 0)
			if (this.display_style)
				this.owner.element.style.display = this.display_style;
		...
		if (message_params.mouse_op_id !== undefined)
			if (this.mouse_operation_id != message_params.mouse_op_id)
				return; // Skip if there have been other mouse operations meanwhile
...
Behave3d.controllerMenu.prototype.createMenuStructure = function(menu_controller, parent_menu)
{
	...
	for (var child_index = 0; child_index < menu_controller.owner.element.childNodes.length; child_index++) {
		...
		if (this.for_each_item)
			this.for_each_item.call(child, this, item_index, first_node, submenu);
...
Behave3d.controllerMenu.prototype.setEventHandlers = function()
{
	...
	if (!this.parent_menu)
	{
		...
		this.active_event_listeners = [
			{element: document, type: "mouseover", handle: focus_handler, useCapture: true, condition: true},
			...
			{element: document,           type: "focus",     handle: focus_handler, useCapture: true, condition: true},
			{element: document,           type: "click",     handle: focus_handler, useCapture: true, condition: true},
		];
...
Behave3d.controllerMenu.prototype.focusEventListener = function(event, is_delegated)
{
	...
	if (event.type == "mouseover") {
		...
		activated_element = event.target;
	else if (event.type == "focus")
		activated_element = document.activeElement;
	
	if (false && !is_delegated)
		console.log("--- event ---- " + this.debugName() + " - type = " + event.type + " - " +
			(activated_element ? " -- activated_element = " + activated_element + "#" + activated_element.id : "") +
			",   menu_is_focused = " + menu_is_focused +
...
Behave3d.controllerMenu.prototype.applyShowButtonStyle = function()
{
	...
	if (!highlight_class && this.parent_menu)
		highlight_class = this.parent_menu.highlight_class;
	
	if (!highlight_class && this.parent_menu && this.parent_menu.owner.element.hasAttribute(this.highlight_class_attribute))
		highlight_class = this.parent_menu.owner.element.getAttribute(this.highlight_class_attribute);	
