...
Behave3d.controllerLookAt.prototype.events         = [
	"start", // Fired when rotation is started after a period of rest
	"end",   // Fired when target angle is reached
];
...
Behave3d.controllerLookAt.prototype.default_params = {
	...
	target_dz       : 0,      // This supplies the Z coordinate of target (relative to this element's Z); currently, no detection is implemented
};
...
Behave3d.controllerLookAt.prototype.update = function()
{
	if (this.paramsHaveChanged())
		this.computed_params = this.getComputedLengths([], [], ['target_dz']);
