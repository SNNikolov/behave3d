...
Behave3d.controllerCarousel.prototype.messages = ["update", // Initializes items of the list
"reset", // Initializes rotation variables
"focus"];
Behave3d.controllerCarousel.prototype.events = ["start", // Triggered on start of rotation animation
...
Behave3d.controllerCarousel.prototype.update = function () {
	...
	if (this.paramsHaveChanged() || !this.computed_params) this.computed_params = this.getComputedLengths(['radius'], [], ['radius_z', 'dz']);
	...
	for (var i = 0; i < this.targets.length; i++) {
		...
		if (this.back_opacity != 1 || this.middle_opacity != 1) {
			var item_opacity = effects_z >= 0 ? 1 - (1 - this.middle_opacity) * (radius_z - effects_z) / radius_z : this.middle_opacity - (this.middle_opacity - this.back_opacity) * -effects_z / radius_z;
...
Behave3d.controllerCarousel.prototype.setItems = function () {
	...
	for (var i = 0; i < this.targets.length; i++) {
		...
		if (this.use_spring) target.physics_enabled = true;
	}
};
