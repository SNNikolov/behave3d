...
Behave3d.controllerOpacity.prototype.events = ["start_fade_in", // Fired upon starting of fade-in transition
...
Behave3d.controllerOpacity.prototype.messages = ["fade_in", // Start a transition from current opacity to 1 (or the value of param this.opacity if different than 1)
...
Behave3d.controllerOpacity.prototype.message = function (message, message_params) {
	...
	if (this.affect == "display" && this.current_opacity != 0 && this.target_opacity == 0) {
	...
	if (this.affect != "" && this.current_opacity == 0 && this.target_opacity != 0)
		// Make targets visible on start of transition
		for (var i = 0; i < this.targets.length; i++) {
			if (this.affect == "visibility") this.targets[i].style.visibility = "visible";else if (this.affect == "display" && this.display_style) this.targets[i].style.display = this.display_style[i];
		}if (instantly) this.update(true);
...
Behave3d.controllerOpacity.prototype.update = function (dont_do_transforms) {
	if (!this.paused && this.frames_left > 0) {
		...
		if (this.frames_left == this.total_frames) {
			if (instantly) this.fireEvent(this.current_action);else this.fireEvent("start_" + this.current_action);
		}
		...
		if (this.frames_left == 0) {
			if (!instantly) this.fireEvent("end_" + this.current_action);

			this.current_opacity = this.target_opacity;
			this.current_action = "";

			if (this.current_opacity == 0) for (var i = 0; i < this.targets.length; i++) {
				if (this.affect == "visibility") this.targets[i].style.visibility = "hidden";else if (this.affect == "display") this.targets[i].style.display = "none";
			}
		}
	}

	if (!dont_do_transforms) this.addTransform({
