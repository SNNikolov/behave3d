...
Behave3d.controllerList.prototype.messages = ["update", // Reads the children of this element in the DOM tree and sets them as items
...
"unselect"];
Behave3d.controllerList.prototype.events = ["update", // Triggered on message "update"; event_params = return object of setChildTargets()
...
Behave3d.controllerList.prototype.message = function (message, message_params) {
	...
	if (message == "update") {
		...
		if (this.focused == -1) this.message("reset");

		if (!this.circular) {
			if (this.focused == 0) this.fireEvent("first_reached");else if (this.focused == this.targets.length - 1) this.fireEvent("last_reached");
		}
		...
		if (this.targets.length == 0) new_focus = -1;else if (message == "focus_pos") new_focus = message_params.pos;else if (message == "focus_first") new_focus = 0;else if (message == "focus_last") new_focus = this.targets.length - 1;else if (message == "focus_next") step = 1;else if (message == "focus_prev") step = -1;else if (message == "focus_step") step = message_params.step;

		if (step != 0) {
			if (this.circular) new_focus = (new_focus + step + this.targets.length) % this.targets.length;else if (new_focus + step >= this.targets.length) this.fireEvent("no_next");else if (new_focus + step < 0) this.fireEvent("no_prev");else new_focus += step;
		}
		...
		if (new_focus != this.focused && this.targets[new_focus]) {
			...
			if (!this.circular) {
				if (old_focus == 0 || old_focus == this.targets.length - 1) this.fireEvent("border_left");
		...
		if (item_to_select >= 0 && item_to_select < this.targets.length) {
			if (message == "select" && pos_in_selected == -1) {
				if (this.select_multiple) this.selected.push(item_to_select);else this.selected = item_to_select;
			} else if (message == "unselect" && pos_in_selected != -1) {
				if (this.select_multiple) this.selected.splice(pos_in_selected);else this.selected = -1;
			}
...
Behave3d.controllerList.prototype.setItems = function () {
	...
	if (this.items == "children") update_results = this.setChildTargets();else Behave3d.debugExit("Behave3d.controllerList.setItems() cannot handle this.items = '" + this.items + "'");

	if (this.focused >= this.targets.length) this.focused = this.targets.length - 1;else if (this.focused < 0) this.focused = this.targets.length > 0 ? 0 : -1;else if (this.targets[this.focused] !== focused_item) {
		var new_focused = this.targets.indexOf(focused_item);

		if (new_focused >= 0) this.focused = new_focused;else if (this.focused >= this.targets.length) this.focused = this.targets.length - 1;else if (this.focused < 0) this.focused = this.targets.length > 0 ? 0 : -1;else {
			...
			for (var i = 0; i < update_results.removed.length; i++) {
				...
				if (this.focused >= removed_block.first_pos && this.focused <= removed_block.last_pos) {
			...
			for (var i = 0; i < update_results.inserted.length; i++) {
				...
				if (inserted_block.first_pos <= new_focused) new_focused += inserted_block.first_pos - inserted_block.last_pos + 1;
			}
