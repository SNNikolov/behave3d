...
Behave3d.controllerListButton.prototype.messages = ["update", // Initializes the button, setting event listeners on the list
"click"];
Behave3d.controllerListButton.prototype.events = ["click", // Fired when the button is clicked or being sent message "click"
...
Behave3d.controllerListButton.prototype.message = function (message, message_params) {
	...
	if (message == "update") {
		...
		if (this.action) {
			var action_params = {};
			if (this.action_pos != -1) action_params.pos = this.action_pos;
			if (this.action_step != 0) action_params.step = this.action_step;
...
Behave3d.controllerListButton.prototype.setEventHandlers = function () {
	...
	if (!this.targets_events_handled) {
		...
		for (var i = 0; i < this.targets.length; i++) {
			this.targets[i].addEventListener("click", function () {
				this_controller.message("click");
			});
		}
	}
};
...
Behave3d.controllerListButton.prototype.doReaction = function (do_what, event_type, do_back) {
	...
	if (!do_back && do_what != "fire" && ["click", "no_prev", "no_next"].indexOf(event_type) >= 0) setTimeout(function () {
	...
	for (var i = 0; i < do_what_parts.length; i++) {
		switch (do_what_parts[i]) {
			case "change_class":
				if (this.alt_class) {
					if (do_back) this.owner.element.classList.remove(this.alt_class);else this.owner.element.classList.add(this.alt_class);
			...
			case "disable":
				...
				break;
			case "fire":
				if (!do_back || event_type == "border_left") this.fireEvent(event_type);
				break;
		}
	}
};
...
Behave3d.controllerListButton.prototype.handlerOfListEvents = function (event_type, event_params) {
	...
	switch (event_type) {
		...
		case "no_prev":
		case "no_next":
			if (this.action != (event_type == "no_prev" ? "focus_next" : "focus_prev")) this.doReaction(this.when_no_progress, "no_progress");
			break;
	}
};
