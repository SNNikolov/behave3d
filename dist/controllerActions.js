...
Behave3d.controllerActions.prototype.messages = ["remove_element", // Remove HTML element from DOM tree; uses Element.removeChild()
"pause", // Pause all controllers of this element
"unpause"];
Behave3d.controllerActions.prototype.default_params = {};

Behave3d.controllerActions.prototype.DOMEvents = ["click", "dblclick", "mouseover", "mouseout", "mousedown", "mouseup", "mousemove", "dragstart", "drag", "dragenter", "dragleave", "dragover", "dragend", "drop", "keydown", "keypress", "keyup", "load", "unload", "abort", "error", "resize", "scroll", "focus", "blur", "focusin", "focusout", "select", "change", "submit", "reset"];
...
Behave3d.controllerActions.prototype.construct = function (params, stage) {
	if (stage == "params") {
		...
		if (this.originalDisplay == "none") this.originalDisplay = "block";
...
Behave3d.controllerActions.prototype.message = function (message, message_params) {
	...
	if (message == "show" && !catches) this.owner.element.style.display = this.originalDisplay;else if (message == "hide" && !catches) {
		this.originalDisplay = window.getComputedStyle(this.owner.element, null).getPropertyValue("display");
...
Behave3d.controllerActions.prototype.addEventHandler = function (event, handler_function) {
	...
	if (use_capture) event = event.substr(0, event.length - 8);

	var handled_events = use_capture ? this.handledDOMEvents_useCapture : this.handledDOMEvents;

	if (this.DOMEvents.indexOf(event) >= 0 && handled_events.indexOf(event) == -1) {
