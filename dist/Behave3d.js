...
Behave3d.consts = {
	...
	DEBUG_STATS_CONTAINER: "" };

// Override constants with properties of object window.behave3dConstants
if (window.behave3dConstants) for (window.behave3dConstants.i in window.behave3dConstants) {
...
Behave3d.startEngine = function () {
	...
	Behave3d.vars.frameTimerID = Behave3d.vars.frameTimerType == "raf" ? window.requestAnimationFrame(Behave3d.runThisFrame) : window.setTimeout(Behave3d.runThisFrame, Behave3d.vars.frameDuration);
...
Behave3d.stopEngine = function () {
	if (Behave3d.var.frameTimerType == "raf") {
		if (window.cancelAnimationFrame) window.cancelAnimationFrame(Behave3d.vars.frameTimerID);
	} else window.clearTimeout(Behave3d.vars.frameTimerID);
};
...
Behave3d.setScene = function (scene_params, scene_controller) {
	if (Array.isArray(scene_params)) scene_params = Behave3d.params.getParamsAsObject(scene_params);
...
Behave3d.updateViewport = function () {
	if (Behave3d.vars.sceneController) Behave3d.vars.sceneController.updateViewport();
};
...
Behave3d.registerController = function (title, constructor) {
	if (Behave3d.controllers[title]) Behave3d.debugOut("Controller with title '" + title + "' is registered again! Mind your references");
	...
	if (constructor.prototype.requires) {
		if (constructor.prototype.requires.controllers) {
			...
			for (var i = 0; i < constructor.prototype.requires.controllers.length; i++) {
				...
				if (!Behave3d[controller_name]) missing.push(controller_name);
			}

			if (missing.length > 0) Behave3d.debugExit("behave3d controller '" + title + "' requires the following controllers: " + missing.join(", "));
		}
	}
...
Behave3d.updatePool = function (skip_removal, behave3d_attribute) {
	...
	for (var i = 0; i < all_behave3d_elements.length; i++) {
		if (!all_behave3d_elements[i][Behave3d.consts.BEHAVE3D_PROPERTY]) new Behave3d.Element(all_behave3d_elements[i], true, behave3d_attribute);
	}Behave3d.doDelayedInits();

	// document.body is the default scene container, if no element with "scene" controller was found
	if (!Behave3d.vars.sceneContainer) Behave3d(document.body).add("scene");
};
...
Behave3d.selector = function (dom_element_or_id, create_behave3d) {
	...
	if (window.jQuery && dom_element_or_id instanceof jQuery) dom_element_or_id = dom_element_or_id[0];else if (typeof dom_element_or_id == "string" && (selector_parts = dom_element_or_id.split(" ")).length == 2) {
	...
	if (return_controller_id != "") {
		if (!element[Behave3d.consts.BEHAVE3D_PROPERTY]) Behave3d.debugExit("Cannot find controller '" + return_controller_id + "' in element " + dom_element_or_id + " - supplied element doesn't have Behave3d.Element");
		return element[Behave3d.consts.BEHAVE3D_PROPERTY].getController(return_controller_id) || Behave3d.debugExit("Cannot find controller '" + return_controller_id + "' in element " + dom_element_or_id);
	}
	// Or return behave3d.Element
	else {
			if (element[Behave3d.consts.BEHAVE3D_PROPERTY]) return element[Behave3d.consts.BEHAVE3D_PROPERTY];
			if (!create_behave3d) Behave3d.debugExit("Element " + dom_element_or_id + " doesn't have Behave3d.Element");
...
Behave3d.doDelayedInits = function () {
	...
	["events", "messages"].forEach(function (mode) {
		for (var i = 0; i < Behave3d.vars.delayedInits.length; i++) {
			...
			if (delayed_inits.controller.construct) delayed_inits.controller.construct(delayed_inits.params, mode);
...
Behave3d.runThisFrame = function () {
	...
	// Show debug info
	if (Behave3d.consts.DEBUG && Behave3d.consts.DEBUG_STATS_CONTAINER) {
		if (!Behave3d.vars.debug_box_ref) {
			Behave3d.vars.debug_box_ref = document.getElementById(Behave3d.consts.DEBUG_STATS_CONTAINER);

			if (Behave3d.vars.debug_box_ref) Behave3d.vars.debug_box_ref.style.visibility = "visible";
		}

		if (Behave3d.vars.debug_box_ref) Behave3d.vars.debug_box_ref.innerHTML = "elements: " + Behave3d.vars.debugInfo.count_behave3d_elements + "<br/>" + "transformed: " + Behave3d.vars.debugInfo.count_transformed_elements + "<br/>" + "translations: " + Behave3d.vars.debugInfo.count_translations + "<br/>" + "rotations: " + Behave3d.vars.debugInfo.count_rotations + "<br/>" + "scalings: " + Behave3d.vars.debugInfo.count_scalings + "<br/>" + "origins: " + Behave3d.vars.debugInfo.count_origins + "<br/>" + "opacities: " + Behave3d.vars.debugInfo.count_opacities + "<br/>" + "frame duration: " + frame_duration.toFixed(1) + "<br/>" + "framerate long-term:  " + Behave3d.vars.longTermFrameDuration.toFixed(1) + "<br/>" + "framerate short-term: " + Behave3d.vars.shortTermFrameDuration.toFixed(1);else Behave3d.debugExit("!");
	}
};
...
Behave3d.params.parseItemsString = function (items_with_params_str, return_as_named_properties) {
	...
	for (var item_i = 0; item_i < items_with_params_str.length; item_i++) {
		...
		if (return_as_named_properties) items[item_title] = item_params;else items.push({
			title: item_title,
			params: item_params
		});
	}
...
Behave3d.params.parseParamsString = function (params_str, return_as_named_properties) {
	...
	for (var param_i = 0; param_i < item_params_strs.length; param_i++) {
		...
		if (param_name_delim_pos != -1) {
			...
			if (Behave3d.params.isNumber(param_val)) param_val = Number(param_val);else if (param_val.toLowerCase() == "true") param_val = true;else if (param_val.toLowerCase() == "false") param_val = false;
		}

		if (return_as_named_properties) item_params[param_name] = param_val;else item_params.push([param_name, param_val]);
	}
...
Behave3d.params.getParam = function (params_array, param_name, do_remove) {
	...
	for (var i = 0; i < params_array.length; i++) {
		if (params_array[i][0] == param_name) {
			...
			if (do_remove) params_array.splice(i, 1);
...
Behave3d.params.extendObject = function (params_object, new_params, only_existing) {
	...
	for (var param_name in params_object) {
		...
		if (!only_existing || result_object.hasOwnProperty(param_name)) result_object[param_name] = new_params[param_name];
	}return result_object;
};
...
Behave3d.params.getLength = function (value, axis, dom_element, allow_non_lengths) {
	...
	if (allow_non_lengths && Array.isArray(allow_non_lengths) && allow_non_lengths.indexOf(value) >= 0) return value;
	...
	if (value_last1 == "X" || value_last1 == "Y" || value_last1 == "Z") {
		...
		value_last1 = value.substr(-1);
	} else if (!axis) Behave3d.debugExit(invalid_notification + "no axis");
	...
	if (value_last3 == "rem") {
		...
		suffix_len = 1;
		total = axis == "X" ? dom_element.parentNode.clientWidth : axis == "Y" ? dom_element.parentNode.clientHeight : Behave3d.vars.sceneParams.perspective;
	} else if (value_last2 == "%p") total = axis == "Y" ? dom_element.parentNode.clientHeight : dom_element.parentNode.clientWidth;else if (value_last2 == "%o") total = axis == "Y" ? dom_element.clientHeight : dom_element.clientWidth;else if (value_last2 == "%d") total = axis == "Y" ? document.documentElement.clientHeight : document.documentElement.clientWidth;else if (value_last2 == "%v") total = axis == "Y" ? window.top.innerHeight : window.top.innerWidth;else if (value_last1 == "o") {
		// Nothing to do
		suffix_len = 1;
		...
		suffix_len = 1;
		if (dom_element) zero = -Behave3d.getElementPos(dom_element, false, true)[axis.toLowerCase()];
	} else if (value_last1 == "v") {
		suffix_len = 1;
		zero = dom_element ? -Behave3d.getElementPos(dom_element, false, true, true)[axis.toLowerCase()] : axis == "Y" ? window.pageYOffset : window.pageXOffset;
	} else if (allow_non_lengths && !Array.isArray(allow_non_lengths)) return value;else Behave3d.debugExit(invalid_notification + "unknown format");

	var number = zero + Number(value.substr(0, value.length - suffix_len));

	if (!Behave3d.params.isNumber(number)) Behave3d.debugExit(invalid_notification + "not a number");
...
Behave3d.getDOMElement = function (element_or_id) {
	...
	if (!element) Behave3d.debugExit("Cannot find HTML element '" + element_or_id + "'");else if (!element.nodeName && !(element instanceof Window) && !(element instanceof Document)) Behave3d.debugExit("HTML element expected, another object type received");
...
Behave3d.getFunctionByName = function (function_name) {
	...
	for (var i = 0; i < namespaces.length; i++) {
		if (!context[namespaces[i]]) return null;else context = context[namespaces[i]];
...
Behave3d.ease = function (value_to_ease, ease_type, ease_amount, total_frames, current_frame, value_type) {
	function get_ease_factor(ease_pos, ease_type) {
		switch (ease_type) {
			case "ease":
				return 0.5 - 0.5 * Math.cos(Math.PI * ease_pos);
			case "ease_in":
				return 1 - Math.cos(0.5 * Math.PI * ease_pos);
			case "ease_out":
				return -Math.cos(0.5 * Math.PI * (1 + ease_pos));
			default:
				Behave3d.debugExit("Unknown ease type '" + ease_type + "' supplied to Behave3d.ease()");
		}
	}
	...
	if (ease_type == "linear" || ease_amount == 0) return value_type == "total" ? value_to_ease * (current_frame + 1) / total_frames : value_to_ease;

	var ease_pos = (current_frame + 1) / total_frames;
	var ease_factor = get_ease_factor(ease_pos, ease_type);

	if (value_type == "current") return value_to_ease * (1 + (ease_factor - 1) * ease_amount);
...
Behave3d.Element = function (element, add_to_pool, behave3d_attribute) {
	...
	if (element.getAttribute && behave3d_attribute) this.add(element.getAttribute(behave3d_attribute));
...
Behave3d.Element.prototype.configureDOMParents = function () {
	...
	while (parent = parent.parentNode) {
		if (parent === Behave3d.vars.sceneContainer || parent === document.body || !parent.style) break;

		if (!parent.style[Behave3d.vars.cssPropTransformStyle]) parent.style[Behave3d.vars.cssPropTransformStyle] = "preserve-3d";
	}
...
Behave3d.Element.prototype.removeFromPool = function () {
	...
	if (index_in_pool < 0 || !this.is_pooled) Behave3d.debugExit(this.debugName() + ".removeFromPool() fails because element is not in the pool");
...
Behave3d.Element.prototype.add = function (controllers_defs, return_references) {
	if (!controllers_defs) return return_references ? [] : this;
	...
	for (var ci = 0; ci < defs_array.length; ci++) {
		...
		if (return_references) references.push(added_controller);
	}

	if (do_events_and_messages) Behave3d.doDelayedInits();
...
Behave3d.Element.prototype.addController = function (controller_or_title, controller_params) {
	...
	if (!Array.isArray(controller_params)) controller_params = Behave3d.params.getParamsAsArray(controller_params);

	var controller;

	// Do not allow more than one actions controller on an element; set parameters to the existing actions controller instead
	if ((controller_or_title === "actions" || controller_or_title === Behave3d.controllerActions) && this.controllers.length > 0) {
		...
		for (var i = 0; i < controller_params.length; i++) {
			if (controller_params[i][0] == "targets") targets_supplied = true;else if (controller_params[i][0] == "owner") owner_supplied = true;
		} // Add "owner" param if not supplied
		if (!owner_supplied) controller_params.unshift(["owner", this]);

		// Add "targets" param if not supplied
		if (!targets_supplied) controller_params.unshift(["targets", [this.element]]);
	...
	if (Behave3d.vars.delayedInits === false) {
		if (controller.construct) controller.construct(controller_params, "events");
		controller.set(controller_params, "events");

		if (controller.construct) controller.construct(controller_params, "messages");
		controller.set(controller_params, "messages");
	} else Behave3d.vars.delayedInits.push({ controller: controller, params: controller_params });
...
Behave3d.Element.prototype.removeController = function (controller) {
	...
	if ((typeof controller === "undefined" ? "undefined" : _typeof(controller)) == "object") controller_index = this.getControllerIndex(controller);else if (Behave3d.params.isNumber(controller)) controller_index = controller;else controller_index = this.getController(controller, true);

	if (!this.controllers[controller_index]) Behave3d.debugExit(this.debugName() + ".removeController(" + ((typeof controller === "undefined" ? "undefined" : _typeof(controller)) == "object" ? controller.debugName() : controller) + ") - cannot find controller in controllers list! controller_index=" + controller_index);
	...
	if (controller.destruct) controller.destruct.call(controller);
...
Behave3d.Element.prototype.getDOMElement = function (element_or_id, return_behave3d) {
	...
	if (typeof element_or_id == "string") element_or_id = element_or_id.toLowerCase();

	if (element_or_id === "@this") dom_element = this.element;else if (element_or_id === "@parent")
		// Set target element to the parentNode in the DOM
		dom_element = this.element.parentNode;else if (element_or_id === "@ancestor") {
	...
	} else dom_element = Behave3d.getDOMElement(element_or_id);
...
Behave3d.Element.prototype.forEachController = function (each_own, each_parent, each_descendant, callback_function) {
	if (each_own) for (var ci = 0; ci < this.controllers.length; ci++) {
		callback_function.call(this.controllers[ci]);
	}if (each_parent) {
		var current_parent;
		while ((current_parent = this.element.parentNode) && current_parent != document) {
			if (current_parent[Behave3d.consts.BEHAVE3D_PROPERTY]) current_parent[Behave3d.consts.BEHAVE3D_PROPERTY].forEachController(true, false, false, callback_function);
		}
	}
	...
	if (each_descendant) {
		...
		for (var child_index = 0; child_index < child_nodes.length; child_index++) {
			...
			if (child[Behave3d.consts.BEHAVE3D_PROPERTY]) child[Behave3d.consts.BEHAVE3D_PROPERTY].forEachController(true, false, true, callback_function);else Behave3d.Element.prototype.forEachController.call(child, false, false, true, callback_function);
		}
	}
};
...
Behave3d.Element.prototype.getController = function (controller_id, get_index) {
	...
	for (var ci = 0; ci < this.controllers.length; ci++) {
		if (this.controllers[ci].id == controller_id) return get_index ? ci : this.controllers[ci];
	}return false;
};
...
Behave3d.Element.prototype.updateControllers = function () {
	if (Behave3d.consts.DEBUG) Behave3d.vars.debugInfo.count_behave3d_elements++;
	...
	while (controllers_list_changed) {
		...
		for (var ci = 0; ci < this.controllers.length; ci++) {
			...
			if (controller.event_handlers.frame) controller.fireEvent("frame", {}, "controller");

			if (!controller.disabled && controller.update) controller.update.call(controller);

			controller.to_be_updated = false;

			// Check if controllers have been added/removed during the update of this controller
			if (controllers_list_changed = controllers_len != this.controllers.length) break;
		}
	}
};
...
Behave3d.Element.prototype.message = function (message, message_params, message_scope) {
	...
	if (send_to_global) for (var ei = 0; ei < Behave3d.vars.elementsPool.length; ei++) {
		...
		var current_parent;
		while ((current_parent = this.element.parentNode) && current_parent != document) {
			if (current_parent[Behave3d.consts.BEHAVE3D_PROPERTY]) sent_messages_counter += current_parent[Behave3d.consts.BEHAVE3D_PROPERTY].message(message, message_params, "local");
		}
	}
	...
	if (send_to_children) {
		...
		for (var child_index = 0; child_index < child_nodes.length; child_index++) {
			...
			} else sent_messages_counter += Behave3d.Element.prototype.message.call(child, message, message_params, "children");
		}
	}
...
Behave3d.Element.prototype.cleanTransforms = function () {
	...
	if (this.physics_enabled) this.acc.x = this.acc.y = this.acc.z = 0;
...
Behave3d.Element.prototype.applyTransforms = function () {
	...
	if (new_origin_str != this.last_origin_str) this.element.style[Behave3d.vars.cssPropTransformOrigin] = this.last_origin_str = new_origin_str;

	// Apply transform-style
	var new_style_str = transform_flatten && transform_flatten.flatten ? "flat" : "preserve-3d";
	if (new_style_str != this.last_style_str) this.element.style[Behave3d.vars.cssPropTransformStyle] = this.last_style_str = new_style_str;

	// Apply opacity
	if (opacity !== this.last_opacity_str) {
		this.element.style.opacity = this.last_opacity_str = opacity;

		if (Behave3d.consts.DEBUG) Behave3d.vars.debugInfo.count_opacities++;
	}
};
...
Behave3d.Element.prototype.calcPhysics = function (add_to_transforms, time_step) {
	...
	if (add_to_transforms) this.transforms.unshift({
...
Behave3d.Element.prototype.fireEvent = function (event_type, event_params, event_scope) {
	...
	if (fire_local && !fire_global) for (var ci = 0; ci < this.controllers.length; ci++) {
		callbacks_called += this.controllers[ci].fireEvent(event_type, event_params, "controller");
	}if (fire_global) for (var ei = 0; ei < Behave3d.vars.elementsPool.length; ei++) {
		callbacks_called += Behave3d.vars.elementsPool[ei][Behave3d.consts.BEHAVE3D_PROPERTY].fireEvent(event_type, event_params, "local");
	}if (fire_parents) {
		var current_parent;
		while ((current_parent = this.element.parentNode) && current_parent != document) {
			if (current_parent[Behave3d.consts.BEHAVE3D_PROPERTY]) callbacks_called += current_parent[Behave3d.consts.BEHAVE3D_PROPERTY].fireEvent(event_type, event_params, "local");
		}
	}
	...
	if (fire_children) {
		...
		for (var child_index = 0; child_index < child_nodes.length; child_index++) {
			...
			} else callbacks_called += Behave3d.Element.prototype.fireEvent.call(child, event_type, event_params, "children");
		}
	}
...
Behave3d.Controller = function (params, enable_physics) {
	...
	// Set params
	if (this.default_params) for (var param_name in this.default_params) {
	...
	if (this.construct) this.construct(params, "params");
};
...
Behave3d.Controller.prototype.setParam = function (param_name, new_val, mode) {
	...
	if (typeof new_val == "string" && new_val.toLowerCase() == "toggle" && this.hasOwnProperty(param_name) && typeof this[param_name] == "boolean") new_val = this[param_name] ? false : true;
	...
	if (param_name == "paused" || param_name == "disabled") {
		...
		if (do_fire_events && new_val != old_val) this.fireEvent(new_val == 1 ? param_name : param_name == "paused" ? "unpaused" : "enabled");

		return true;
	} else if (param_name.substr(-3) == " on") {
		// Create auto-event handler for "xxxx on" param
		if (do_auto_handlers) this.on(new_val, param_name.substr(0, param_name.length - 3).trim());
		return this;
	} else if (Behave3d.Controller.common_messages.indexOf(param_name) >= 0 || this.messages && this.messages.indexOf(param_name) >= 0) {
		// Send message for params with names that are messages
		if (do_messages) this.message(param_name, new_val);
		return this;
	} else if (this instanceof Behave3d.controllerActions && !this.hasOwnProperty(param_name)) {
	...
	if (do_params) {
		if (["construct", "destruct", "message", "update"].indexOf(param_name) >= 0) {
			...
			if (typeof func_ref == "function") this[param_name] = func_ref;else Behave3d.debugExit("Controller " + this.debugName() + " received invalid param (" + new_val + ") that cannot override method " + param_name + "(), function or function name is expected.");
		} else if (this.hasOwnProperty(param_name)) {
			if (this instanceof Behave3d.controllerActions && param_name == "id" && new_val != "a") {
		...
		} else if (!unknown_params_ok) Behave3d.debugExit("Controller " + this.debugName() + " received unknown param '" + param_name + "'");
	}
...
Behave3d.Controller.prototype.set = function (new_params, mode) {
	...
	if (Array.isArray(new_params)) for (var i = 0; i < new_params.length; i++) {
		this.setParam(new_params[i][0], new_params[i][1], mode);
	} else {
		if (typeof new_params == "string") new_params = Behave3d.params.parseParamsString(new_params, true);
...
Behave3d.Controller.prototype.setMessageParams = function (message, message_params) {
	if (message_params === undefined) message_params = {};else if ((typeof message_params === "undefined" ? "undefined" : _typeof(message_params)) == "object") this.set(message_params, "message");
...
Behave3d.Controller.prototype.handleCommonMessage = function (message, message_params) {
	...
	if (this.messages && this.messages.indexOf(message) >= 0)
	...
	if (message == "pause" || message == "unpause") this.set({ paused: message == "pause" });else if (message == "enable" || message == "disable") this.set({ disabled: message == "disable" });else if (message == "params") this.set(message_params);else if (message == "remove") this.owner.removeController(this);else Behave3d.debugExit(this.debugName() + " received unknown message '" + message + "'");
...
Behave3d.Controller.prototype.on = function (event_to_wait_for, handler_function_or_message, return_targets) {
	...
	if (target_path.length == 1) {
		if (Behave3d.params.isNumber(target_path[0])) {
			delay = Number(target_path[0]);
			target.event = true;
		} else target.event = target_path[0];
		...
		if (target_path[0] == "@targets") for (var i = 0; i < this.targets.length; i++) {
			target_elements.push(this.getTarget(i));
		} else target_elements.push(this.owner.getDOMElement(target_path[0], true));
	...
	var handler_function = typeof handler_function_or_message == "string" ? function (event_type, event_params) {
		this.message(handler_function_or_message, event_params);
	}.bind(this) : handler_function_or_message.bind(this);

	var delayed_handler_function = delay ? function (event_type, event_params) {
		window.setTimeout(function () {
			handler_function(event_type, event_params);
		}, delay);
	} : handler_function;
	...
	for (var i = 0; i < targets.length; i++) {
		...
		} else targets[i].controller.addEventHandler(targets[i].event, delayed_handler_function);
	}if (return_targets) return targets.length > 1 ? targets : targets[0];else return this;
};
...
Behave3d.Controller.prototype.off = function (event, handler_function) {
	if (Array.isArray(event)) for (var i = 0; i < event.length; i++) {
		this.off(event[i], handler_function);
	}if (handler_function) this.removeEventHandler(event, handler_function);else
...
Behave3d.Controller.prototype.removeEventHandler = function (event, handler_function) {
	...
	if (handlers.indexOf(handler_function) >= 0) {
		for (var i = handlers.length - 1; i >= 0; i--) {
			if (handlers[i] === handler_function) handlers.splice(i, 1);
...
Behave3d.Controller.prototype.fireEvent = function (event_type, event_params, event_scope) {
	...
	if (event_scope == "controller") {
		var handlers = (this.event_handlers[event_type] ? this.event_handlers[event_type] : []).concat(this.event_handlers["all"] ? this.event_handlers["all"] : []);
	...
	} else return this.owner.fireEvent(event_type, event_params, event_scope);
};
...
Behave3d.Controller.prototype.getAnotherController = function (controller_id) {
	...
	if (controller_path.length == 1) return this.owner.getController(controller_id) || Behave3d.debugExit(this.debugName + " cannot find another controller with id = '" + controller_id + "'");else return Behave3d(controller_id, false);
};
...
Behave3d.Controller.prototype.makeTargetsDom3d = function (add_to_pool) {
	...
	for (var target_index = 0; target_index < this.targets.length; target_index++) {
		...
		if (!target[Behave3d.consts.BEHAVE3D_PROPERTY]) new Behave3d.Element(target, add_to_pool);
	}
...
Behave3d.Controller.prototype.registerActions = function (actions_target, action_handles, event_handles) {
	...
	if (typeof actions_target == "string" && actions_target.length > 0) {
		...
		element = at_parts[1] || "";
	} else if (typeof actions_target == "number") delay = actions_target;
	...
	if (typeof action_handles == "string") {
		...
		if (which_actions.indexOf("showhide") >= 0) actions_list = actions_list.concat(["show", "hide", "show_immediately", "hide_immediately"]);
	...
	if (reverse) {
		for (var action in action_handles) {
			if (action == "show") swap_props(action_handles, "show", "hide");else if (action == "show_immediately") swap_props(action_handles, "show_immediately", "hide_immediately");
		}for (var action_event in event_handles) {
			if (action_event == "show_start") swap_props(event_handles, "show_start", "hide_start");else if (action_event == "show_end") swap_props(event_handles, "show_end", "hide_end");
		}
	}
	...
	if (typeof event_handles == "string") {
		...
		if (which_events.indexOf("showhide") >= 0) events_list = events_list.concat(["show_start", "show_end", "hide_start", "hide_end"]);
...
Behave3d.Controller.prototype.setChildTargets = function (number_limit, add_to_pool) {
	...
	for (var i = 0; i < this.owner.element.childNodes.length; i++) {
		...
		if (child.nodeType == Node.ELEMENT_NODE) new_targets.push(child);
	}

	// Remove old targets which are not among new_targets
	for (var i = 0; i < this.targets.length; i++) {
		if (this.targets[i] != this.owner.element && new_targets.indexOf(this.targets[i]) == -1) {
...
Behave3d.Controller.prototype.addTransform = function (transform, target_index) {
	if (target_index === undefined) for (var ti = 0; ti < this.targets.length; ti++) {
		this.getTarget(ti).addTransform(transform);
	} else this.getTarget(target_index).addTransform(transform);
...
Behave3d.Controller.prototype.debugName = function () {
	return (this.owner ? this.owner.debugName() : "UnassignedController") + ":" + this.id + "(#" + this.unique_id + " " + this.title + ")";
};
...
Behave3d.StepEngine = function (vars, is_circular, events_controller, options) {
	...
	for (var option_name in Behave3d.StepEngine.default_options) {
		if (this.options[option_name] === undefined) this.options[option_name] = Behave3d.StepEngine.default_options[option_name];
	}if (this.is_circular) {
...
//---------------------------------------
Behave3d.StepEngine.events = ["start", "half", "end", "start_back", "half_back", "end_back", "pos0", "pos1", "cycle", "mark", "cycle_back", "mark_back"];
...
Behave3d.StepEngine.prototype.start = function (movement_direction, from_beginning, is_new_movement, movement, duration) {
	...
	if (movement) this.setMovement(movement, duration, is_new_movement);
	...
	if (from_beginning && this.movement_start) for (var var_name in this.vars) {
	...
	if (this.pos < 0) this.pos = 0;else if (this.pos > 1) this.pos = 1;
	...
	if (this.movement_dpos > 0 || this.options.always_fire_start_end) {
		...
		if (this.total_frames == 0) this.update(false);
...
Behave3d.StepEngine.prototype.update = function (is_paused) {
	...
	if (!is_paused && this.direction != 0) {
		...
		for (var var_name in this.vars) {
			...
			this.vars[var_name] = this.current_start[var_name] + Behave3d.ease(this.current_movement[var_name], this.current_ease, this.options.ease_amount, this.total_frames, this.total_frames - this.frames_left, "total");
		}if (this.events_controller && this.frames_left == this.total_frames) this.events_controller.fireEvent("start" + this.events_suffix);

		if (this.events_controller && this.frames_left == Math.floor(this.total_frames / 2)) this.events_controller.fireEvent("half" + this.events_suffix);

		if (this.is_circular) {
			if (this.options.cycle_len > 0 && this.frames_left < this.total_frames) {
				var var_name = this.var_names[0];
				if (Behave3d.isAnglePassed(this.starting_angle, this.vars[var_name], prev_vars[var_name], this.options.cycle_len)) this.events_controller.fireEvent("cycle" + this.events_suffix);
			}

			if (this.options.marked_angle != -1) {
				var var_name = this.var_names[0];
				if (Behave3d.isAnglePassed(this.options.marked_angle, this.vars[var_name], prev_vars[var_name], this.options.cycle_len)) this.events_controller.fireEvent("mark" + this.events_suffix);
			}
		}
	...
	if (this.options.half_step != 1) for (var var_name in this.vars) {
	...
	if (this.options.half_step == 1 && this.options.spring_acc == 0) for (var var_name in this.vars) {
