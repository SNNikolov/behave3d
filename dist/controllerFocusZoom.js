...
Behave3d.controllerFocusZoom.prototype.events = ["focus", // Fired upon message "focus"
"blur"];
Behave3d.controllerFocusZoom.prototype.messages = ["focus", // Tell the element is focused
"blur"];
...
Behave3d.controllerFocusZoom.prototype.construct = function (params, stage) {
	if (stage == "params") {
		...
		if (this.target_id != "") this.targets = [this.owner.getDOMElement(this.target_id)];
...
Behave3d.controllerFocusZoom.prototype.update = function () {
	if (this.paramsHaveChanged() || !this.computed_params) this.computed_params = this.getComputedLengths(['dx'], ['dy'], ['dz']);

	if (this.is_focused) this.vpos += (1 - this.current_pos) * this.speed_acc;else this.vpos -= this.current_pos * this.speed_acc;

	this.vpos = this.vpos * this.speed_dampening; // Dampening of spring motion

	if (!this.paused) this.current_pos += this.vpos;
